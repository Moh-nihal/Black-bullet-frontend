...
const t = (val) => typeof val === 'object' && val !== null ? val.en || '' : val || '';
      ...
      <main className="flex-1 w-full overflow-x-hidden overflow-y-auto bg-surface min-h-screen">
        ...
        <header className="flex flex-wrap justify-between items-center w-full gap-x-4 gap-y-4 px-4 pl-14 md:px-8 lg:px-12 py-4 md:py-6 sticky top-0 z-50 bg-background/60 backdrop-blur-xl border-b border-white/10">
          <div className="order-1 flex items-center gap-4 md:gap-8 w-auto h-8">
            <h2 className="font-headline text-[15px] md:text-2xl font-black tracking-widest text-black uppercase leading-none mt-1">Service Inventory</h2>
          </div>
        ...
        <div className="p-8 lg:p-12 space-y-12">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div className="space-y-2">
              ...
              <h3 className="font-headline text-4xl lg:text-5xl font-black text-black uppercase tracking-tighter">Performance Catalog</h3>
            </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-0 border border-white/5">
            {[
              ...
              <div key={s.label} className={`bg-surface-container p-6 lg:p-8 space-y-1 ${i < 3 ? "border-r border-white/5" : ""}`}>
                ...
                <p className={`font-headline text-2xl lg:text-3xl font-bold ${s.highlight ? "text-primary" : "text-black"}`}>{s.val}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 mb-4">
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full lg:w-auto lg:justify-end">
              <div className="relative w-full md:w-64 shrink-0">
                ...
                <input 
                  className="bg-surface-container-highest border-none text-xs font-label uppercase tracking-widest px-10 py-2 w-full focus:ring-1 focus:ring-primary placeholder:text-on-surface-variant/70" 
                ...
                {isLoading ? (
                  ...
                  data?.data?.map((r) => (
                    <tr key={r._id} className="group hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="p-4 lg:p-6">
                        <span className="font-headline text-sm font-bold tracking-wider text-black">{t(r.title)}</span>
                        <p className="text-[10px] text-on-surface-variant mt-1">{r.slug}</p>
                      </td>
                      <td className="p-4 lg:p-6"><span className="text-xs font-label uppercase text-on-surface-variant">{t(r.category) || "General"}</span></td>
                      <td className="p-4 lg:p-6"><span className="font-headline text-sm font-bold text-black">AED {r.price || 0}</span></td>
                      <td className="p-4 lg:p-6">
                      ...
                      <td className="p-4 lg:p-6 text-right space-x-2">
                        <Link href={`/admin/services/new?id=${r._id}`} className="p-2 inline-block hover:bg-black/5 text-on-surface-variant hover:text-black transition-all"><span className="material-symbols-outlined text-sm">edit</span></Link>
