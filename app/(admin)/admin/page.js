...
const getBookingColors = (statusRaw) => {
  ...
  return { statusColor: "bg-black/10 text-black", dotColor: "bg-black" };
};
      ...
      <main className="flex-1 w-full overflow-x-hidden overflow-y-auto bg-surface min-h-screen">
        ...
        <header className="flex flex-wrap justify-between items-center w-full gap-x-4 gap-y-4 px-4 pl-14 md:px-8 py-4 sticky top-0 z-50 bg-background/60 backdrop-blur-xl border-b border-white/10">
          <div className="order-1 flex items-center gap-4 md:gap-8 w-auto h-8">
            <h1 className="text-[15px] md:text-xl font-black tracking-widest text-black uppercase font-headline leading-none mt-1">
              Operations Overview
            </h1>
          </div>
          ...
          <div className="order-3 w-full md:w-auto md:flex-1 md:order-1 md:ml-8 relative">
            ...
            <input
              className="bg-surface-container-highest border-none text-xs text-black px-10 py-2 w-full md:w-64 focus:ring-0 focus:border-b-2 focus:border-primary transition-all font-label uppercase tracking-widest placeholder:text-on-surface-variant/70"
        ...
        <div className="p-8 space-y-10">
              ...
              <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-surface-container-highest p-6 relative overflow-hidden flex flex-col justify-center">
                  <div className="absolute top-0 right-0 p-4">
                    <span className="material-symbols-outlined text-black/5 text-6xl">trending_up</span>
                  </div>
                  <p className="font-headline text-xs font-bold uppercase tracking-[0.1em] text-on-surface-variant mb-2">Revenue Velocity</p>
                  <h3 className="text-2xl font-headline font-bold text-black italic tracking-tighter uppercase">{data?.metrics?.revenueVelocityLabel || "Steady Flow"}</h3>
          ...
          <section className="bg-surface-container">
                      ...
                      return (
                        <tr key={b.id} className="hover:bg-white/5 transition-colors group">
                          <td className="px-8 py-5">
                            <p className="text-sm font-bold text-black">{b.client || "Guest"}</p>
                          ...
                          <td className="px-8 py-5">
                            <Link href={`/admin/bookings?id=${b.id}`} className="text-black opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="px-8 py-4 border-t border-white/5 flex justify-center">
              <Link href="/admin/bookings" className="text-[10px] font-headline font-bold uppercase tracking-widest text-on-surface-variant hover:text-black transition-colors">
