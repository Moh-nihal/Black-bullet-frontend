...
import { applyCmsAutoTranslate } from "@/lib/cmsAutoTranslate";
...
const initialServicesData = {
  ...
  ecuDetail: {
    ...
    bookingCtaText: "Book This Service",
    bookingCtaTextAr: "",
...
const initialGalleryData = {
  ...
  featured: {
    ...
    afterImage: null,
    videoSrc: null,
    videoLabel: "Process: Carbon Kit Install",
  ...
  gridItems: [
    { type: "image", src: "/images/gallery-porsche.jpg", alt: "Silver Porsche 911 GT3 driving", title: "Track Precision Setup", subtitle: "Porsche 911 GT3", category: "Performance", className: "masonry-item-tall" },
    { type: "image", src: "/images/gallery-steering.jpg", alt: "Close up of a luxury car steering wheel", title: "", subtitle: "", category: "Detailing", className: "masonry-item-square" },
    { type: "image", src: "/images/gallery-lambo.jpg", alt: "Lamborghini Aventador front view", title: "THE BULL OF DUBAI", subtitle: "Aventador SVJ Performance Kit", category: "Wrapping", className: "masonry-item-wide" },
    { type: "image", src: "/images/gallery-amg.jpg", alt: "Mercedes AMG engine details", title: "", subtitle: "", category: "Performance", className: "masonry-item-square" },
    { type: "image", src: "/images/gallery-supra.jpg", alt: "Custom white Supra drifting", title: "Supra MK5 Hybrid", subtitle: "Drift Spec", category: "Performance", className: "masonry-item-tall" },
    { type: "image", src: "/images/gallery-red.jpg", alt: "Red Ferrari detail", title: "", subtitle: "", category: "Detailing", className: "masonry-item-square" },
    { type: "image", src: "/images/gallery-exhaust.jpg", alt: "Performance car exhaust with flames", title: "", subtitle: "Live from Workshop", category: "Performance", className: "masonry-item-wide" },
  ],
  cta: {
...
export default function ContentManagementPage() {
  ...
  const loadContent = useCallback(async (pageKey) => {
    ...
    try {
      ...
      if (apiData && Object.keys(apiData).length > 0) {
        if (pageKey === "settings") {
          tabConfig[pageKey]?.setData({ ...initialSettingsData, ...apiData });
        } else {
  ...
  const handleAutoTranslate = async () => {
    ...
    if (!config) return;
    setTranslating(true);
    const tid = toast.loading("Translating English → Arabic…");
    try {
      const next = await applyCmsAutoTranslate(activeTab, config.data);
      config.setData(next);
      toast.success("Arabic fields updated — review and save.", { id: tid });
    } catch (err) {
      toast.error(err?.message || "Translation failed", { id: tid });
    } finally {
      ...
      <main className="flex-1 w-full overflow-x-hidden overflow-y-auto bg-surface min-h-screen relative">
        ...
        <header className="flex justify-between items-center w-full px-4 pl-14 md:px-8 py-4 sticky top-0 z-50 bg-background/60 backdrop-blur-xl border-b border-white/10">
          <div className="flex items-center gap-8">
            <h1 className="text-xl font-black tracking-widest text-black uppercase font-headline">
              Website Content
            </h1>
          </div>
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="flex items-center gap-2 text-primary hover:brightness-110 disabled:opacity-50 transition-all font-label text-[10px] font-bold uppercase tracking-widest shrink-0"
            >
              <span className="material-symbols-outlined text-base">translate</span>
              <span className="max-[480px]:sr-only">{translating ? "Translating…" : "Auto-translate EN → AR"}</span>
              <span className="hidden max-[480px]:inline">{translating ? "…" : "AR"}</span>
            </button>
