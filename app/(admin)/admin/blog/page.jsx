        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center w-full px-4 pl-14 md:px-8 lg:px-10 py-4 md:py-6 lg:py-8 sticky top-0 z-30 bg-background/60 backdrop-blur-xl border-b border-white/5 gap-4">
          ...
          <div className="flex items-center gap-4 md:gap-6 w-full md:w-auto">
            <div className="relative hidden lg:block">
              ...
              <input 
                className="bg-surface-container-highest border-none text-xs font-label tracking-widest py-3 pl-10 pr-4 w-64 focus:ring-0 placeholder:text-on-surface-variant/70" 
                ...
                return (
                  <div key={card._id} className={`group bg-surface-container relative ${isDraft ? "border border-white/5" : ""}`}>
                    <div className={`aspect-video overflow-hidden relative ${isDraft ? "opacity-60" : ""}`}>
                      ...
                      {!isDraft && card.category && (
                        <div className="absolute top-0 right-0 p-4">
                          <span className="bg-primary text-on-primary-fixed text-[10px] font-black uppercase px-3 py-1 tracking-widest">{t(card.category)}</span>
                        </div>
                      )}
                      {isDraft && (
                    ...
                    <div className="p-6">
                      ...
                      <h5 className={`font-headline text-lg font-bold uppercase mb-4 group-hover:text-primary transition-colors leading-tight ${isDraft ? "text-on-surface-variant/60" : "text-on-surface"}`}>{t(card.title)}</h5>
                      <p className={`text-sm ${isDraft ? "text-on-surface-variant/40" : "text-on-surface-variant"} line-clamp-2 mb-8 font-light`}>{t(card.shortDesc) || "No description provided."}</p>
